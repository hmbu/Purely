...
| M-02 Cancel order? | G-06 → "Cancel order" (shown only while the status is New) | "Yes, cancel" → request sent. Four outcomes: cancelled → G-06 with status Cancelled; already accepted → G-06 with the real status and its "already accepted" banner; the hotel holds no order with this number → M-02 closes with no copy and G-06 shows its own "Order not found" state; any other failure → error text inside M-02 with the same two buttons. "No, keep my order" / backdrop tap → close, G-06 unchanged |
